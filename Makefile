...
build:
	go build -ldflags="-s -w -X github.com/webrpc/ridl-lsp.VERSION=$$(git describe --tags --always 2>/dev/null || echo dev)" -o ./bin/ridl-lsp ./cmd/ridl-lsp

install:
	go install -ldflags="-s -w -X github.com/webrpc/ridl-lsp.VERSION=$$(git describe --tags --always 2>/dev/null || echo dev)" ./cmd/ridl-lsp
